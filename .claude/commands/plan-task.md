...
4. **Design.** Launch `feature-dev:code-architect` agent(s) (same subagent as `/feature-dev` Phase 4) for the implementation approach. One agent is enough when the approach is obvious; use 2-3 in parallel with different trade-off focuses (minimal change, clean architecture, pragmatic balance) only when the approach is genuinely ambiguous.
