...
| Entity | Fields | Notes |
|---|---|---|
| `Restaurant` | `id`, `name`, `type` (`food`\|`drink`), `contact_info`, `menu_source_note` | A restaurant only ever serves food or drink, so `type` is set once at creation, not per item (moved here from `MenuItem` in task 015). `menu_source_note` is free text — link or note on where the menu image/link came from. |
