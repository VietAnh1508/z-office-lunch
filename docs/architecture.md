...
- **`/admin` password gate (task 043): client-side only, checked against a Vite build-time env var (`VITE_ADMIN_PASSWORD`).** Chosen over a Worker-secret + API-enforced check for simplicity — no new endpoint, no session/cookie handling. The trade-off is accepted deliberately: the password is baked into the shipped JS bundle and readable via devtools/view-source by anyone, so this is a deterrent against casual URL-wandering, not real access control. Admin API endpoints stay reachable directly (curl/devtools) with no password, unchanged. Real access control is still the documented Cloudflare Access follow-up (`docs/deployment.md`).
  - **Amendment (task 051): the check moved server-side.** `POST /api/admin/verify-password` now checks the submitted password against an `ADMIN_PASSWORD` Worker secret, never shipped to the client. The `sessionStorage` boolean, no-session-token, and admin-API-endpoints-still-unauthenticated trade-offs above are otherwise unchanged — this task closed specifically the "password readable via devtools" gap, not the others.
