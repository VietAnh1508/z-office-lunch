...
- The e2e spec initially failed `tsc --noEmit` at the repo root: an explicit `HTMLInputElement` type annotation on a `page.evaluate` callback doesn't resolve, because the root `tsconfig.json` (which covers `e2e/`, `vitest.config.ts`, `playwright.config.ts`) inherits `"lib": ["ES2022"]` from `tsconfig.base.json` with no `dom` lib. First worked around this with a structural cast (`el as { validity: { valid: boolean } }`) instead of naming `HTMLInputElement` directly. On a follow-up question from the user about whether the root cause could be fixed instead of worked around, added `"lib": ["ES2022", "DOM"]` directly to the root `tsconfig.json`'s `compilerOptions` — scoped only to that file's own `include` (`e2e`, `vitest.config.ts`, `playwright.config.ts`), so it doesn't affect `apps/web`, `apps/api`, or `packages/db`, which each already have their own independent tsconfig (and `apps/web/tsconfig.app.json` already opts into `"lib": ["ES2023", "DOM"]` on its own). Reverted the structural cast back to a plain `HTMLInputElement` annotation once the lib fix was in place and re-verified `pnpm typecheck && pnpm --filter web build && pnpm test && pnpm exec playwright test` all still pass. Nothing else about the plan changed.
