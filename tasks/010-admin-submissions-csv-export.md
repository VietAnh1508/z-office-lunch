...
- fix commit: `f5b1ff5` — manual review against real MS Excel found the exported CSV opened as one unsplit column (comma isn't the default delimiter Excel expects in all locales, including Vietnamese). Added a failing test asserting a `sep=,` directive line, confirmed it failed against the green implementation, then fixed `toCsv()` to emit it. `pnpm -r typecheck && pnpm --filter web build && pnpm test` -> all passing (224/224 tests) after the fix.
...
- The code-reviewer pass (below) didn't catch a real Excel-compatibility bug: the AC's "opening cleanly in Excel" was verified only via automated round-trip parsing (which necessarily assumes comma is the delimiter), not by actually opening the file in Excel. The user caught it by opening the exported file in real MS Excel and finding the columns weren't split (Excel was using its locale's list separator, semicolon in this case, to parse the file instead of comma). Fixed by prepending a `sep=,` directive line, which forces comma-splitting regardless of locale. Worth remembering for any future CSV-export work: automated tests can confirm a CSV round-trips through its own parser without confirming a real spreadsheet app opens it the way you expect.
