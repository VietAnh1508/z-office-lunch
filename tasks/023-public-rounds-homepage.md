...
- follow-up (post-review, user-requested public navbar — see Plan Deviations): red `169b19d`, green `7595ed2`, fix `d3dbced` — `pnpm -r typecheck && pnpm --filter web build && pnpm test` -> all passing (191 tests, 20 files)
...
- After the review above, the user asked (in chat, after the PR was already open) for a header/navbar to appear on public pages too — not in the task's Goal/Acceptance Criteria/Plan at all. Folded it into this same PR at the user's explicit request rather than opening a new task, since it's a small, directly-related follow-up to the page this task just built (added `PublicLayout` wrapping the public routes, TDD red `169b19d` → green `7595ed2`). The user then spotted, from a live screenshot, that the new public header rendered a different height than `AdminLayout`'s — root cause was `AdminLayout`'s nav links carrying their own vertical padding (`py-1.5`) that the plain public brand link didn't have, so the two headers' content rows differed in height even though their outer padding matched. Fixed by extracting both into a shared `AppHeader` component (`apps/web/src/components/AppHeader.tsx`) with an explicit `h-9` content row and an optional `nav` slot, so the two layouts can no longer drift apart (commit `d3dbced`) — done as a single fix commit rather than a further red/green split, since it changed no user-facing behavior already covered by the `PublicLayout`/`AdminLayout` tests, only a rendering detail (verified visually via the Chrome dev tool against both `/` and `/admin`, plus the full `test_command` re-run).
